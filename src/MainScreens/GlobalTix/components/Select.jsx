...
const Select = (props) => {
  ...
  const [value, setValue] = useState(single ? undefined : []);
  ...
  const hasValue = single ? (value !== undefined && value !== null && value !== '') : (value && value.length > 0);
  const showClearIcon = Boolean(allowClear && hasValue);
  const showArrowIcon = Boolean(!showClearIcon);
    ...
    <>
      ...
      <>
        <BottomSheetComponent
          name={name}
          renderSelect={(renderValueResult) => (
            <View
              style={[
                styles.main,
                {
            ...
            >
              <View style={styles.body}>{renderValueResult}</View>
              <View>
                {loading ? (
